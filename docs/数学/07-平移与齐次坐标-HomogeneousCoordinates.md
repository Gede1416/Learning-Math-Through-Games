...
远端还保留了扩展练习 `Homework/数学/07-平移齐次坐标-扩展/Mat4x4.cs`，其中包含矩阵乘法与 `T·R` 组合，作为 Day 3 的备用材料；本轮默认测试不启用它。

### 扩展练习验收（2026-09-02）

第一次单独运行 `Mat4x4` 的六项测试为 **5/6 PASS**。挂点案例期望 `(10,1,-1.3)`、实际 `(1.3,1,10)`；当时 TODO 6 旋转了角色的世界位置，却没有旋转局部剑尖偏移。正确关系是：
...
- `RotateY(+90°)` 满足 `+Z→+X`、`+X→-Z`。
- 基向量测试改为 `RotateY(+90°)·(0,0,5)=(5,0,0)`。

学生最终实现的 `RotateY` 和 `TransformPoint` 都符合该约定；独立复验结果为 **6/6 PASS**，剑尖世界位置为 `(10,1,-1.3)`。
