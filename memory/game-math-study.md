...
**正在进行**：阶段一 Day 4 → 矩阵与向量乘法（Primer Ch 4）。学生第二次回答正确：原代码得到 `(0,-1)`、期望 `(0,1)`。已讲解 `Mv` 的每个分量是矩阵对应行与向量的点积、矩阵列是变换后的基轴、原代码等价于 `Mᵀv`，并区分存储主序与乘法约定。已布置 Matrix2x2.cs 两个 TODO、5 项测试，等待作业验收。
...
**How to apply:** 按用户节奏推进，每次聚焦一个概念。当前阶段一 Day 4 矩阵与向量乘法，等待学生回答坏代码问题后再给教材定义、推导、标准解和作业。
