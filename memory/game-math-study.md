...
**正在进行**：阶段二 Day 4 → 模型/世界/相机坐标系转换。已给出世界转相机坏代码：相机在 `(10,0,0)`、绕 Y `+90°` 面向世界 `+X`，目标 `(15,0,0)`；代码减去相机位置后仍应用 local→world 的正向旋转，导致前方目标落到局部 `-Z`。当前只运行第九轮 3 项测试，等待学生手算并识别 world→camera 应使用相机旋转的逆；尚未展示标准解。
