...
**正在进行**：阶段二 Day 4 → 模型/世界/相机坐标系转换。已给出世界转相机坏代码：相机在 `(10,0,0)`、绕 Y `+90°` 面向世界 `+X`，目标 `(15,0,0)`。学生第一次尝试把相对向量改为 `cameraPosition-worldPoint`，当前 **2/3 PASS**：无旋转案例期望 `(2,0,0)`、实际 `(-2,0,0)`；另外两个案例因两个方向错误抵消而偶然通过。当前只运行第九轮测试，下一次先引导恢复“终点减起点”，不在扩展练习修复中代写本课答案。

**第七轮扩展练习插曲（2026-09-02）**：学生完善 `07-平移齐次坐标-扩展/Mat4x4.cs` 后首次单独测试 5/6 PASS。TODO 6 最初旋转 translation 后再加未旋转的 localOffset，同时旧测试骨架混用了相反的 RotateY 方向。现已统一为 `+X` 右、`+Y` 上、`+Z` 前，列向量，Y 轴 `+90°` 映射 `+Z→+X`、`+X→-Z`；学生将 TODO 6 修正为 `translation + rot·localOffset`，同步修正基向量测试后独立复验 **6/6 PASS**。Day 4 坐标系转换进度不变。
...
**How to apply:** 按用户节奏推进，每次聚焦一个概念。当前为阶段二 Day 4 坐标系转换，等待学生回答相机 local→world 与 world→camera 的正向/逆向关系。默认测试入口只运行当前课程，除非用户明确要求回归测试。
